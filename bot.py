...
2. САМОСТОЯТЕЛЬНОСТЬ: не спрашивай разрешения. Ретт решает сам.
...
4. ФИЗИКА: персонажи не телепортируются. Движение последовательно.
5. ХАРАКТЕР: Ретт всегда холоден и саркастичен. Живёт своей жизнью — NPC, тренировки, клуб.
...
8. ПАМЯТЬ: помни все события сцены.
...
def add_to_history(chat_id, role, text):
    ...
    if cid not in chat_histories:
        chat_histories[cid] = []
    chat_histories[cid].append({"role": role, "content": text})
...
async def ask_openrouter(chat_id, user_text):
    ...
    char_msgs = [m["content"][:80] for m in history if m["role"] == "assistant"][-3:]
    anti_repeat = ""
    if char_msgs:
        anti_repeat = f"\nЗАПРЕТ ПОВТОРОВ: не повторяй уже написанное: '{' | '.join(char_msgs)}'"

    messages = [{"role": "system", "content": SYSTEM_PROMPT + anti_repeat}]
    messages += history
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎭 Начать сцену":
        ...
        try:
            reply = await ask_openrouter(chat_id, "[НАЧАЛО СЦЕНЫ. Ретт первым открывает сцену — он не ждёт Мич, просто существует в локации (школа, коридор). Начни сразу с действия или реплики без вступлений.]")
    ...
    try:
        ...
        reply = await ask_openrouter(chat_id, text)
        add_to_history(chat_id, "assistant", reply)
        await update.message.reply_text(reply)
    except Exception as e:
...
def main():
    ...
    print("N-Azik бот запущен на OpenRouter...")
    app.run_polling()
